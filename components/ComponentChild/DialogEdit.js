...
const DiaglogEdit = ({ message, onEdit, onCancle, onDelete, visible, title, setValue, value, idFind }) => {
    return (
        <Dialog
            visible={visible}
            title={title}
        ...
        >
            <View style={styles.container}>
                <View style={{alignItems:"flex-start", justifyContent:"flex-start",width:"100%"}}>
                    <TextInput placeholder='Type something...' style={styles.input} defaultValue={value} onChangeText={text=>setValue(text)}/>
                </View>
                <TouchableOpacity onPress={() => {onEdit(idFind, value) }} style={styles.button}>
                    <Text style={{ color: "#ffffff" }}>Edit task</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => {onDelete(idFind, value) }} style={styles.button}>
